...
import { QrCode, CheckCircle, XCircle, AlertCircle, Clock, User, Home, Camera, CameraOff, Sunrise, Sun, Moon } from 'lucide-react';
...
const MEAL_ICONS: Record<MealType, typeof Sunrise> = { breakfast: Sunrise, lunch: Sun, dinner: Moon };

const REMAINING_FIELD: Record<MealType, 'remaining_breakfast' | 'remaining_lunch' | 'remaining_dinner'> = {
  breakfast: 'remaining_breakfast',
  lunch: 'remaining_lunch',
  dinner: 'remaining_dinner',
};
...
export default function StaffScanner() {
  ...
  async function processQR(qrData: string) {
    ...
    try {
      ...
      const graceEnd = sub.grace_end_date || sub.end_date;
      if (today > graceEnd) {
        setResult({ success: false, message: 'Your subscription and grace period have expired.', student, subscription: sub, mealType });
        setScanning(false);
        return;
      }
      ...
      const remainingField = REMAINING_FIELD[mealType];
      const remainingCount = sub[remainingField];

      if (typeof remainingCount === 'number' && remainingCount <= 0) {
        const label = mealType.charAt(0).toUpperCase() + mealType.slice(1);
        setResult({ success: false, message: `No ${label} meals remaining.`, student, subscription: sub, mealType });
        setScanning(false);
        return;
      }
      ...
      // Decrease only this meal type's balance — nothing else changes.
      if (typeof remainingCount === 'number') {
        await supabase
          .from('subscriptions')
          .update({ [remainingField]: remainingCount - 1 })
          .eq('id', sub.id);
      }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <PageHeader title="Meal Scanner" subtitle="Scan student QR codes to grant meals" />

      {/* Meal Type Selector */}
      <SectionCard title="Select Meal Session">
          ...
          {(['breakfast', 'lunch', 'dinner'] as MealType[]).map(type => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1.5 py-3 rounded-xl capitalize font-semibold text-sm transition-all duration-200 ${
                  active ? 'bg-red-600 text-white shadow-md shadow-red-600/25' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                }`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          actions={
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
                cameraActive ? 'bg-red-100 text-red-700 hover:bg-red-200' : 'bg-red-600 text-white hover:bg-red-700'
              }`}
            >
        ...
        >
          ...
          {cameraError && (
            <p className="text-xs text-red-600 bg-red-50 rounded-xl p-2 mb-3">{cameraError}</p>
          )}
        ...
        <div className="space-y-4">
          {result ? (
            <div className={`card border-2 ${result.success ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className={`font-bold text-lg ${result.success ? 'text-green-800' : result.message === 'Meal Already Taken' ? 'text-orange-800' : 'text-red-800'}`}>
                    {result.message}
                  </h3>
              ...
              {result.student && (
                <div className="space-y-3 pt-4 border-t border-gray-200">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 avatar-gradient rounded-xl flex items-center justify-center">
                      <span className="text-white font-bold">{result.student.full_name.charAt(0)}</span>
                    </div>
                    <div>
                      <p className="font-bold text-slate-900">{result.student.full_name}</p>
            ...
            <div className="card">
              <EmptyState icon={QrCode} title="Scan result will appear here" />
            </div>
          )}
          ...
          <div className="card">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-slate-900 text-sm capitalize">Today's Count — {mealType}</h3>
            </div>
      ...
      {/* Scan History */}
      <SectionCard title={`Recent Scans — ${mealType}`} actions={<span className="text-sm text-gray-500">{history.length} scans today</span>}>
        {loadingHistory ? (
          <div className="space-y-2 animate-pulse">
            {[1, 2, 3].map(i => <div key={i} className="h-12 bg-gray-100 rounded-xl" />)}
          </div>
        ) : history.length === 0 ? (
          <EmptyState icon={Clock} title={`No scans yet for ${mealType}`} />
        ) : (
            ...
            {history.map(log => (
              <div key={log.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-slate-900 text-sm truncate">{(log.student as any)?.full_name}</p>
